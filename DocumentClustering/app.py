...
def main():
	...
	try:
		...
		conn = mysql.connector.connect(host='localhost', database='visual', user='root', password='')
...
#					years_dict[value] = [key]
	...
	plotdata = [plot + [data[1]] for plot, data in zip(pos.tolist(), rows)]
	
	
	# render the data to the frontend by sending it
	return render_template('home.html', rows=d, descriptions=rows, plotdata = plotdata)
